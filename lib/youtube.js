...
exports.create = function(path, contentType){
  return new Upload(path, contentType);
};
...
function Upload(path, contentType) {
  this.meta = {};
  this.id = path;
  this.path = path;
...
Upload.prototype.contentType = function (contentType) {
...
Upload.prototype.token = function (token) {
...
Upload.prototype.uploadData = function(fn){
  ...
  url = parseUrl(url);
  options.method = 'POST';
  options.path =  '/feeds/api/users/' + user + '/uploads'; //url.pathname;
  options.host = 'uploads.gdata.youtube.com'; //url.hostname;
  options.port = 80;
  ...
  fs.stat(path, function(err, stat){
    ...
    len += stat.size;
    buf('\r\n--' + boundary + '\r\n');
    buf('Content-Type: '+ self.__contentType +'\r\n');
...
Upload.prototype.parseXML = function(res, fn){
  ...
  parser.on('end', function(obj){
    ...
    obj.thumbnails = [];
    for(var t in obj['media:group']['media:thumbnail']) {
      obj.thumbnails.push(obj['media:group']['media:thumbnail'][t]['@']);
    }
